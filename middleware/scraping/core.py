...
class Mediator:
    ...
    # async def run(self, playwright, base_timespan=BASE_TIMESPAN, logout_timespan=LOGOUT_TIMESPAN, query_symbol: str = None, source_url: str = "https://bcs-express.ru", query_span: str = "category?tag=/{/}", trim="/{/}", extracted_news: News = []):
    #     print("__chromium_launch__")
    #     chromium = playwright.chromium  # or "firefox" or "webkit".
    #     browser = chromium.launch()
    #     page = browser.new_page()
    #     base_url = source_url + query_span.replace(trim, query_symbol) if query_symbol else source_url
    #     print("source:" ,base_url)
    #     page.goto(base_url)
    #     print("__page_goto__")
    #     time.sleep(randint(base_timespan, logout_timespan))

    #     # Wait for the div element to be present
    #     page.wait_for_selector('.feed__list')

    #     # Extract all child elements of the div
    #     child_elements = page.query_selector_all('.feed__list > *')
    ...
    #         link_element = element.query_selector('a.feed-item__head')
    #         link_href = link_element.get_attribute('href')

    #         date_text = element.query_selector('.feed-item__date').inner_text()
    #         title_text = element.query_selector(
    #             '.feed-item__title').inner_text()
    #         summary_text = element.query_selector(
    #             '.feed-item__summary').inner_text()

    #         child_page = browser.new_page()
    #         child_page.goto(source_url + link_href)
    #         print("__child_page_goto__")
    #         time.sleep(randint(base_timespan, logout_timespan))

    #         child_page.wait_for_selector('div[data-id="publication-content"]')

    #         # Extract the HTML content of the targeted div element
    #         targeted_div_content = child_page.inner_html(
    #             'div[data-id="publication-content"]')
    ...
    #         extracted_news.append(News.from_orm(NewsBase(link_href=link_href, date_text=date_text, title_text=title_text, summary_text=summary_text, publication_in=date_trim())))

    #     browser.close()
    #     extracted_news = sorted(extracted_news, key=sorting_by_publication_date)
