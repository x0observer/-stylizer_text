...
class OpenAIService:
    ...
    def generate_response_by_prompt(prompt: str, openai_key: str = os.environ["OPENAI_API_KEY"], unicode: Optional[str] = 'utf-8') -> str:
        ...
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты помощник, который отвечает на запросы пользователя."},
    ...
    def summarize_news(news: str, summarize_prefix: str = "Пожалуйста, сократи новость и вырази ее суть в нескольких предложениях не теряя важную информацию", openai_key: str = os.environ["OPENAI_API_KEY"], unicode: Optional[str] = 'utf-8') -> str:
        ...
        finite_prompt = f"{summarize_prefix}: {news}"

        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты помощник, который сокращает новости."},
                {"role": "user", "content": finite_prompt}
            ],
            max_tokens=1025,
            n=1,
            temperature=0.7
    ...
    @staticmethod
    def summarize_news_conclusions(summarize_news_conclusions: List[str], stock_title: str, stock_info: str, summarize_prefix: str = "Пожалуйста, сократи новость и вырази ее суть в нескольких предложениях не теряя важную информацию", openai_key: str = os.environ["OPENAI_API_KEY"], unicode: Optional[str] = 'utf-8') -> str:
        print("openai_key: ", openai_key)
        
        concatenated_news = "\n".join(summarize_news_conclusions)
        finite_prompt = f"{summarize_prefix}: {concatenated_news}"

        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Ты помощник, который анализирует и сокращает новости."},
                {"role": "user", "content": finite_prompt}
            ],
            max_tokens=1025,
            n=1,
